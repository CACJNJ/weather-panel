...
// ============================================================
const STATIC_UPDATED = '2026-08-07';

// ---------- ENSO 总览【静态/人工维护 · 官方指数快照，来源核对: 2026-08-07】 ----------
// 数值均取自下方各项 sources 列出的固定权威来源；无法核实者标注"未核实"，不凭记忆/模型生成
// 2026-08-07 核对结果：四项官方源逐一重查，本日均无新发布 → 数值全部维持，仅核对日推进。
//   CPC NAO 仍为 7月 −0.3134（8/6 首次发布，月度指数，8月值预计9月初）；
...
//   下周为密集发布窗口：BOM 约 8/11、NOAA 8/13，两者均可能带来实质变化。
const SOURCE_CHECKED = '2026-08-07';
const ensoOverview = [
...
const alerts = [
  {
    ...
    cadence:'每周指数更新 · 每月官方讨论', obsPeriod:'BOM周值至 2026-07-26 / NOAA讨论 2026-07-09', updated:'BOM 2026-07-28；NOAA 2026-07-09（8/5重查仍无新发布，下期约 BOM 8/11、NOAA 8/13）', checked:'2026-08-07',
  ...
  },
  {
    ...
    cadence:'月度官方更新', obsPeriod:'月值 2026-06（最新可得）', updated:'JMA 2026-06 值', checked:'2026-08-07',
  ...
  },
  {
    ...
    cadence:'月度更新', obsPeriod:'月值 2026-07（本次新发布）', updated:'NOAA CPC 2026-07 值（8/6核对时新增）', checked:'2026-08-07',
  ...
  },
  {
    ...
    cadence:'每两周更新', obsPeriod:'至 2026-07-26', updated:'BOM 2026-07-28（新）', checked:'2026-08-07',
    sources:[{l:'BOM IOD',u:'https://www.bom.gov.au/climate/iod/'}]
  },
];

// ---------- 海洋指数【静态/人工维护 · ENSO/PDO/NAO/IOD, 每项标观测期与来源, 核对 2026-08-07】 ----------
const oceanIndices = [
  {
    ...
    cadence:'每周指数 / 每月讨论', obsPeriod:'BOM周值至7/26 · NOAA讨论7/9', checked:'2026-08-07',
  ...
  },
  {
    ...
    cadence:'月度官方更新', obsPeriod:'月值 2026-06（最新可得）', checked:'2026-08-07',
    sources:[{l:'JMA 月度PDO',u:'https://ds.data.jma.go.jp/tcc/tcc/products/elnino/decadal/pdo_month.html'}]
  },
  {
    ...
    cadence:'月度更新', obsPeriod:'月值 2026-07（本次新发布）', checked:'2026-08-07',
    sources:[{l:'NOAA CPC NAO',u:'https://www.cpc.ncep.noaa.gov/products/precip/CWlink/pna/nao.shtml'}]
  },
  {
    ...
    cadence:'每两周更新', obsPeriod:'至 2026-07-26', checked:'2026-08-07',
...
// ⚠️ 下期 Crop Progress 为 8/10 发布(数据截至8/9)，在此之前美国各行的百分比维持8/2口径不变，仅"风险"列随天气/市场更新；
...
const cropCalendar = [
  ...
  { crop:'🌽 玉米', region:'美国玉米带', season:'乳熟43%、凹陷6%(截至8/2，均快于5年均值38%/5%)', sensitive:'灌浆定粒期水分与高温', risk:'优良率61%(8/2)创季内新低、低于去年73%；48州表层墒情短缺48%未改善。授粉已收尾，粒重由8月水分与灌浆期温度共同决定；需求端新增隐忧：饲牛/活牛8/6重挫，饲用需求预期转弱', enso:'CPC连续三期(8/4、8/5、8/6发布)指向偏热，窗口后移至8/12-8/20；但玉米带无偏干指引，属"偏热未必偏干"' },
  { crop:'🫘 大豆', region:'中国东北', season:'开花-结荚期', sensitive:'结荚鼓粒期水分', risk:'进入产量形成关键期，8月水分为核心变量', enso:'关注8月干旱风险' },
  { crop:'🫘 大豆', region:'美国中西部', season:'结荚62%(截至8/2，快于5年均值55%)', sensitive:'鼓粒期温度(本品种当前最敏感项)', risk:'优良率63%(8/2)持平前周、未随玉米下滑；<b>但CPC连续三期确认的8/12-8/20偏热窗口完整覆盖鼓粒期</b>，是全表当前最集中的单一风险——鼓粒期高温直接压制单粒重与蛋白', enso:'中国新作累计采购278万吨、承诺2500万吨提供需求支撑；巴西出口纪录1.154亿吨压制' },
...
const AG_NEWS = [
  {
    title: '★偏热信号连续第三期确认，且CPC自己用了"持续"一词：8月6日发布的6-10天(有效期8/12-8/16)与8-14天(8/14-8/20)展望写道——"在6-10天时段，美国大部地区<b>继续</b>倾向于偏高气温"。三期同向已基本排除单次模式噪音，且窗口随发布日推移而整体后移至8/12-8/20，恰好完整覆盖美豆鼓粒期与玉米灌浆后段。需同时注意：偏少降水信号仅指向墨西哥湾沿岸与佛罗里达(受副热带高压脊影响)，玉米带并无偏干指引——即"偏热但未必偏干"，与7月那轮又热又旱的组合性质不同',
    link: 'https://www.cpc.ncep.noaa.gov/products/predictions/610day/fxus06.html',
    ts: '2026-08-07T06:30:00+08:00', date: '8/7 06:30', source: 'NOAA CPC 预报讨论原文', sourceClass: 'usda',
  },
  {
    title: '周四(8/6)玉米大豆止跌企稳、小麦接棒下跌：9月玉米4.39(+2¼)结束三连跌，9月大豆11.60(+3½)、豆粕311.60(+1.40)、豆油67.74(+2点)；但<b>9月芝加哥小麦大跌11美分至6.31¼</b>，与前一日的逆势收高完全反向。12月棉花83.16(+14点)连续第三日上涨。板块内部轮动加快：三日内玉米、小麦轮流领跌，棉花独自走强，尚未形成一致方向',
    link: 'https://www.brownfieldagnews.com/market-news/closing-grain-and-livestock-futures-august-6-2026/',
    ts: '2026-08-07T04:10:00+08:00', date: '8/7 04:10', source: 'Brownfield Ag News', sourceClass: 'usda',
  },
  {
    title: '★畜牧板块大幅下挫值得单独关注：10月活牛224.92(−4.55)、9月饲牛341.57(−6.80)，跌幅在近期属显著级别；10月瘦肉猪81.72(−1.30)。饲牛与活牛同步重挫通常反映育肥利润预期恶化或存栏节奏变化，对中期玉米饲用需求为负面信号——在美玉米出口销售本就疲弱的背景下，需求端两条腿(出口、饲用)同时走弱的风险上升',
    link: 'https://www.brownfieldagnews.com/market-news/closing-grain-and-livestock-futures-august-6-2026/',
    ts: '2026-08-07T04:15:00+08:00', date: '8/7 04:15', source: 'Brownfield Ag News', sourceClass: 'usda',
  },
  {
    title: '宏观转向：9月原油反弹2.07美元至77.29，结束此前三日累计约9.6美元的跌势；但道指回落464点至53,885，黄金基本持平于4,306.70。即前一日"股强商弱"的组合在本日反转为"商品回稳、权益回吐"，宏观资金方向仍不明朗，谷物暂难从中获得稳定的增量买盘',
    link: 'https://www.brownfieldagnews.com/market-news/closing-grain-and-livestock-futures-august-6-2026/',
    ts: '2026-08-07T04:20:00+08:00', date: '8/7 04:20', source: 'Brownfield Ag News', sourceClass: 'usda',
  },
  {
    title: '下周为密集事件窗口，三项叠加：①周一(8/10)USDA作物生长报告，验证降水能否让玉米优良率自61%的季内低位企稳；②<b>8月WASDE(8/12)首次纳入实地调查单产</b>，是本月最大单一事件风险；③气候层面 BOM 约8/11、NOAA 8/13 相继发布，均可能带来实质变化。特别提示：WASDE 恰好落在 CPC 所指的8/12-8/20偏热窗口起点，若报告单产偏高而其后高温兑现，市场可能在同一周内先后消化两个方向相反的冲击',
    link: 'https://www.nass.usda.gov/Publications/National_Crop_Progress/',
    ts: '2026-08-07T06:50:00+08:00', date: '8/7 06:50', source: '本站综合 · USDA / NOAA / BOM', sourceClass: 'usda',
  },
];

// ============================================================
// 特殊天气事件 — 台风/高温/强降水/霜冻等重大天气（静态参考, 更新于 2026-08-07）
